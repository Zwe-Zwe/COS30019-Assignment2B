...
def build_model(num_classes: int, freeze_backbone: bool = True) -> nn.Module:
    ...
    # ViT head structure in torchvision:
    # (heads): Sequential(
    #   (head): Linear(in_features=768, out_features=1000, bias=True)
    # )
    in_features = model.heads.head.in_features
    model.heads = nn.Sequential(
        nn.Dropout(0.1), # Standard dropout for ViT heads
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Transfer learning using pretrained ViT-B/16"
    )
    ...
    logger = RunLogger("transfer_vit", args.log_dir)
    ...
    # For ViT, if unfreezing, we might want to be careful, but we'll stick to the standard logic
    # If frozen, only optimize heads. If unfrozen, optimize all.
    params = model.parameters() if not args.freeze_backbone else model.heads.parameters()
    ...
    ckpt_path = args.output_dir / "transfer_vit.pth"
